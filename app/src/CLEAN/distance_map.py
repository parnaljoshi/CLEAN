...
def get_cluster_center(model_emb, ec_id_dict):
    ...
    with torch.no_grad():
        for ec in tqdm(list(ec_id_dict.keys())):
            if '-' in ec or len(ec.split('.')) != 4:
                continue
            ids = ec_id_dict[ec]
            emb_cluster = model_emb[id_counter:id_counter + len(ids)]
...
def dist_map_helper(keys1, lookup1, keys2, lookup2):
    ...
    for i, key1 in tqdm(enumerate(keys1), total=len(keys1)):
        ...
        dist_norm = (current - lookup2).norm(dim=1, p=2).detach().cpu().numpy()
        dist[key1] = {key2: dist_norm[j] for j, key2 in enumerate(keys2)}
    return dist
...
def dist_map_helper_dot(keys1, lookup1, keys2, lookup2):
    ...
    for i, key1 in tqdm(enumerate(keys1), total=len(keys1)):
        current = lookup1[i].unsqueeze(0)
        dist_norm = ((current - lookup2).norm(dim=1, p=2) ** 2).detach().cpu().numpy()
        dist[key1] = {key2: dist_norm[j] for j, key2 in enumerate(keys2)}
    return dist
...
def get_dist_map(ec_id_dict, esm_emb, device, dtype, model=None, dot=False):
    ...
    model_lookup = torch.stack([cluster_center_model[ec].to(device=device, dtype=dtype) for ec in ecs])
    print(f'Calculating distance map, number of EC4 clusters: {len(ecs)}')
    return dist_map_helper_dot(ecs, model_lookup, ecs, model_lookup) if dot else dist_map_helper(ecs, model_lookup, ecs, model_lookup)

def get_dist_map_test(train_emb, test_emb, ec_id_dict_train, id_ec_test, device, dtype, dot=False):
    cluster_center_model = get_cluster_center(train_emb, ec_id_dict_train)
    ecs = list(cluster_center_model.keys())
    model_lookup = torch.stack([cluster_center_model[ec].to(device=device, dtype=dtype) for ec in ecs])
    ids = list(id_ec_test.keys())
    print(f'Calculating eval distance map: {len(ids)} test IDs vs {len(ecs)} EC4 clusters')
    return dist_map_helper_dot(ids, test_emb, ecs, model_lookup) if dot else dist_map_helper(ids, test_emb, ecs, model_lookup)

def get_random_nk_dist_map(train_emb, rand_emb, ec_id_dict_train, rand_ids, device, dtype, dot=False):
    cluster_center_model = get_cluster_center(train_emb, ec_id_dict_train)
    ecs = list(cluster_center_model.keys())
    model_lookup = torch.stack([cluster_center_model[ec].to(device=device, dtype=dtype) for ec in ecs])
    return dist_map_helper_dot(rand_ids, rand_emb, ecs, model_lookup) if dot else dist_map_helper(rand_ids, rand_emb, ecs, model_lookup)
